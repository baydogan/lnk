...
func TestAPIKeyCreateAndGetByHash(t *testing.T) {
	...
	k := &domain.APIKey{KeyHash: "hash1", Prefix: "lnk_abc12345"}
...
func TestAPIKeyGetByHashNotFound(t *testing.T) {
	clearCollection(t, "api_keys")
	repo := NewAPIKeyRepository(testDB)
	if _, err := repo.GetByHash(context.Background(), "nope"); !errors.Is(err, domain.ErrNotFound) {
...
func TestAPIKeyCount(t *testing.T) {
	...
	if err := repo.Create(ctx, &domain.APIKey{KeyHash: "h", Prefix: "p"}); err != nil {
		t.Fatalf("Create: %v", err)
	}
...
func TestAPIKeyDuplicateHash(t *testing.T) {
	...
	first := &domain.APIKey{KeyHash: "same", Prefix: "p1"}
	if err := repo.Create(ctx, first); err != nil {
		t.Fatalf("first Create: %v", err)
	}
	dup := &domain.APIKey{KeyHash: "same", Prefix: "p2"}
...
func TestAPIKeyOneKeyPerNilUser(t *testing.T) {
	...
	if err := repo.Create(ctx, &domain.APIKey{KeyHash: "h1", Prefix: "p1"}); err != nil {
		t.Fatalf("first Create: %v", err)
	}
	err := repo.Create(ctx, &domain.APIKey{KeyHash: "h2", Prefix: "p2"})
...
func TestAPIKeyDeleteByUserID(t *testing.T) {
	...
	if err := repo.Create(ctx, &domain.APIKey{KeyHash: "h1", Prefix: "p1", UserID: &uid}); err != nil {
		t.Fatalf("Create: %v", err)
	}
	if err := repo.Create(ctx, &domain.APIKey{KeyHash: "h2", Prefix: "p2", UserID: &other}); err != nil {
		t.Fatalf("Create: %v", err)
	}
	...
	if _, err := repo.GetByHash(ctx, "h1"); !errors.Is(err, domain.ErrNotFound) {
